...
double pump,stokes;
double E_field(double t),pump0,E_fild(double t),stokes0;
...
int main(void)
{
    ...
    while(t<tmax){  /****** main loop *********/
        ...
        pump0=1.0*E_field(t);
        stokes0=1.0*E_fild(t);
        stokes1=1.0*E_field1(t);/*0.6*/
        pump1=1.0*E_fild1(t);
        pump2=1.0*E_field2(t);
        stokes2=1.0*E_fild2(t);
        pump=pump0+pump1+0.0*pump2;
        stokes=stokes0+stokes1+0.0*stokes2;
        /**************************************************/
        //val=Fluctn(t);
        ...
        H[3][2]=0.5*stokes1;H[3][3]=0.0*(delta)/*off two resonance*/;
...
double E_fild (double t){
    ...
    E= (E0*cos(PI/4)*exp( -(t-t1)*(t-t1)/(sigmaT*sigmaT) ))+(E0*exp( -(t+t1)*(t+t1)/(sigmaT*sigmaT) ));
...
double E_fild1 (double t){
    ...
    t1=-13.7*sigma  ;/*-13.7*sigma*/
    
    
    E= (E0*cos(PI/2)*exp( -(t-t1)*(t-t1)/(sigmaT*sigmaT) ))+(E0*exp( -(t+t1)*(t+t1)/(sigmaT*sigmaT) ));
